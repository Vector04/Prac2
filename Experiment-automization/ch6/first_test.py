...
device = ArduinoVISADevice(port='ASRLCOM5::INSTR')
...
for i,V_in in enumerate(V_ins):
    ...
    for ii in range(m):
        v1s[i, ii] = device.measure_input_value(channel=1)
        v2s[i, ii] = device.measure_input_value(channel=2)
        time.sleep(0.05)
...
device.set_output_voltage(voltage=0, channel=0)
device.device.close()
...
# pd.DataFrame({"V_0": V_ins, "V_1": v1s, "dV_1": dv1s, "V_2": v2s, "dV_2": dv2s}).to_csv("data22.csv", index=False)
